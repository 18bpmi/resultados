...
const isAdmin = perfil => perfil?.perfil === "admin" || isPermanentAdmin(perfil?.email);
...
async function carregarUsuarios(){
  ...
  $("usuariosBody").innerHTML = linhas.map(([uid,u]) => {
    const principal=isPermanentAdmin(u.email);
...
onAuthStateChanged(auth, async user => {
  ...
  if(!perfilAdmin?.ativo || !isAdmin(perfilAdmin)) return location.replace("index.html");
  if(isPermanentAdmin(user.email) && perfilAdmin.perfil!=="admin") {
